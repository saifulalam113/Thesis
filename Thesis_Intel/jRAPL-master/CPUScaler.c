...
JNIEXPORT jint JNICALL Java_Hello_ProfileInit(JNIEnv *env, jclass jcls) {
	jintArray result;
	int i;
	char msr_filename[BUFSIZ];
...
JNIEXPORT jint JNICALL Java_Hello_GetSocketNum(JNIEnv *env, jclass jcls) {
	return (jint)getSocketNum();
}



JNIEXPORT jstring JNICALL Java_Hello_EnergyStatCheck(JNIEnv *env,
		jclass jcls) {
	jstring ener_string;
	char gpu_buffer[num_pkg][60]; 
	char dram_buffer[num_pkg][60]; 
...
JNIEXPORT void JNICALL Java_Hello_ProfileDealloc
   (JNIEnv * env, jclass jcls) {
	int i;
	free(fd);	
	free(parameters);
